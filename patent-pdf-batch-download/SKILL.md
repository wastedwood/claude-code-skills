...
1. 使用完整专利名称搜索。
2. 结果过多时，增加申请人或申请年份缩小范围。
3. 打开最强候选的专利详情页。
4. 在详情页核对申请号、申请日、标题和申请人。
5. 只有详情页申请号规范化后与源表完全一致，才接受该公开号。

严禁只凭标题接受候选，因为标题可能重复，例如“一种变频一体机”。标题只是寻找候选的手段，申请号才是最终匹配依据。

如果 Google Patents 经过上述流程仍无法找到可验证候选，才将国家知识产权局检索结果页作为备用来源。使用备用来源时逐行整体读取公开号和申请号，并用规范化申请号精确匹配。
...
- Google Patents 申请号搜索无结果：改用“专利名称 + 申请人或年份”寻找候选，并在详情页精确核对申请号。
- Google Patents 仍无法验证：说明具体失败项；用户允许时再使用国家知识产权局作为备用来源。
